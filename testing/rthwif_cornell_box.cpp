...
void* build_rtas(sycl::device device, sycl::context context, sycl::queue queue, std::vector<ze_rtas_builder_geometry_info_exp_t*>& descs)
{
  /* get L0 handles */
  ...
  if (use_device_memory)
  {
    ...
    if (std::holds_alternative<ze_command_list_handle_t>(hQueue))
    {
      ...
      err = ZeWrapper::zeRTASBuilderCommandListAppendCopyExp(hCommandList, accelDevice, accelHost, accelBytes, hEvent, 0, nullptr);
      if (err != ZE_RESULT_SUCCESS)
        throw std::runtime_error("zeRTASBuilderCommandListAppendCopyExp failed");
...
void* build_tlas(sycl::device device, sycl::context context, sycl::queue queue, void* blas)
{
  ...
  ze_rtas_builder_instance_geometry_info_exp_t inst = {};
  inst.geometryType = ZE_RTAS_BUILDER_GEOMETRY_TYPE_EXP_INSTANCE;
  inst.instanceFlags = 0;
  inst.geometryMask = 0xFF;
  inst.instanceUserID = 0;
  inst.transformFormat = ZE_RTAS_BUILDER_INPUT_DATA_FORMAT_EXP_FLOAT3X4_ALIGNED_COLUMN_MAJOR;
  ...
  /* fill geometry descriptor array with pointer to single geometry descriptor */
  std::vector<ze_rtas_builder_geometry_info_exp_t*> descs;
  descs.push_back((ze_rtas_builder_geometry_info_exp_t*)&inst);
